...
class C3VDDataset(data.Dataset):
    ...
    def __init__(self,
        ...
        if self.sequences is not None:
            self.video_files = [
                vf for vf in self.video_files
                if os.path.basename(vf) in self.sequences]
            missing = self.sequences - {os.path.basename(vf) for vf in self.video_files}
            if missing:
                raise ValueError("C3VD sequences not found: {}".format(sorted(missing)))
        ...
        seq_names = sorted({scan["sequence"] for scan in self.scans})
        print("Prepared C3VD dataset with %d frames from %d sequences: %s" % (
            len(self.scans), len(seq_names), ", ".join(seq_names)))
