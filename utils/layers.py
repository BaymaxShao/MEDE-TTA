...
def masked_mean(tensor, mask, eps=1e-7):
    """Mean of tensor values where mask is active; returns 0 when mask is empty."""
    weight = mask.float()
    denom = weight.sum()
    if denom < eps:
        return tensor.sum() * 0.0
    return (tensor * weight).sum() / denom
...
def get_smooth_bright(transform, target, pred, occu_mask):
    ...
    eps = 1e-7
    loss_x = grad_transform_x.sum() / mask_x.sum().clamp(min=eps)
    loss_y = grad_transform_y.sum() / mask_y.sum().clamp(min=eps)
    if mask_x.sum() < eps and mask_y.sum() < eps:
        return grad_transform_x.sum() * 0.0
...
class get_occu_mask_backward(nn.Module):
    ...
    def forward(self, flow, th=0.95):
        ...
        max_disp = max(flow.shape[-2], flow.shape[-1])
        flow = flow.clamp(-max_disp, max_disp)
        new_locs = self.grid + flow
        new_locs = new_locs[:, [1,0], ...]
...
class get_occu_mask_bidirection(nn.Module):
    ...
    def forward(self, flow12, flow21, scale=0.01, bias=0.5):
        flow12 = torch.nan_to_num(flow12, nan=0.0)
        flow21 = torch.nan_to_num(flow21, nan=0.0)
        max_flow = max(flow12.shape[-2], flow12.shape[-1])
        flow12 = flow12.clamp(-max_flow, max_flow)
        flow21 = flow21.clamp(-max_flow, max_flow)
...
def get_corresponding_map(data):
    ...
    x = data[:, 0, :, :].view(B, -1)
    y = data[:, 1, :, :].view(B, -1)
    x = torch.nan_to_num(x, nan=0.0, posinf=float(W - 1), neginf=0.0)
    y = torch.nan_to_num(y, nan=0.0, posinf=float(H - 1), neginf=0.0)
